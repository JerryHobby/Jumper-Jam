...
func write(type:Type, msg:String):
	...
	for line:String in _log_msgs:
		if (line.contains(type_prefix[Type.DEBUG]) and ShowDEBUG) \
		or (line.contains(type_prefix[Type.INFO]) and ShowINFO) \
		or (line.contains(type_prefix[Type.ERROR]) and ShowERROR):
			new_text += line + "\n"
...
func print_log(my_prefix = ""):
	for line in _log_msgs:
		print(my_prefix + line)
